...
with right:
    st.subheader("💡 ตัวอย่างคำถาม (กดเพื่อใส่อัตโนมัติ)")
    c1, c2, c3 = st.columns(3)
    if c1.button("ทำครัว งบ 3000 minimal", use_container_width=True):
        st.session_state.prefill = "ทำครัว งบ 3000 minimal"
    if c2.button("งบ 2500 ปูพื้นภายนอก modern", use_container_width=True):
        st.session_state.prefill = "งบ 2500 ปูพื้นภายนอก modern"
    if c3.button("ขอหินแกรนิตที่ถูกที่สุด", use_container_width=True):
        st.session_state.prefill = "ขอหินแกรนิตที่ถูกที่สุด"

    st.write("")
    st.caption("ถามความรู้ก็ได้ เช่น: หินแกรนิตกับหินอ่อนต่างกันยังไง")
